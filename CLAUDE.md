...
| `design/m7_m8_m15_execution_plan.md` | **Execution plan for the three open milestones** — the order to work M7's 16, M8's 1 and M15's 14 open issues, and **how many parallel workers that order can actually keep busy: 5, plus a serialized lane for #303/#306 and a lane that does nothing but review.** 31 issues collapse to **7 waves, average width 4.1, peak 6** — which is the real cap, not compute. **CI is not the bottleneck** (measured 3m30s–4m per code PR, per-branch concurrency so parallel PRs never cancel each other); the review gate, three contended files (`streak_ring_widget.dart`, `macro_summary_card.dart`, `add_meal_bottom_sheet.dart`) and the Actions job ceiling are. Carries the hard gates, the five corrections the audit produced (Epic #11 called three issues parallelisable that share two files; **#98 was unimplementable and was rewritten**), what every worker is told, and what the plan explicitly does not claim. **Read before assigning any M7, M8 or M15 issue** |
...
16 open issues with a build order in `design/backlog_handoff.md` §3 — see
...
| M7 — Polish | `epic:m7-polish` | #88–#94, #151, #301–#311 | 19 — **16 open** |
