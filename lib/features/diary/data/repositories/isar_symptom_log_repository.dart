...
class IsarSymptomLogRepository implements SymptomLogRepository {
  ...
  @override
  Future<SymptomLog> save(SymptomLog log) => guardPersistence(
    'IsarSymptomLogRepository.save',
    () async {
      final schema = SymptomLogMapper.toIsar(log);
      // Replaces whatever record already holds this date, and writes the
      // resulting id back into `schema` in place — so a log saved with a
      // null id comes back carrying the row it landed on.
  ...
  Future<SymptomLog?> findByDate(DateTime date) =>
      guardPersistence('IsarSymptomLogRepository.findByDate', () async {
  ...
  Future<List<SymptomLog>> findAll() =>
      guardPersistence('IsarSymptomLogRepository.findAll', () async {
  ...
  Future<void> deleteByDate(DateTime date) =>
      guardPersistence('IsarSymptomLogRepository.deleteByDate', () async {
