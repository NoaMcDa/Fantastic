...
void main() {
  ...
  testWidgets('selection is single — a second choice replaces the first', (
    tester,
  ) async {
    await pumpScreen(tester);
  ...
  // Not a toggle: tapping the chosen card again must not drop back to no
  // answer and silently disable the CTA the user is reaching for.
  testWidgets('tapping the selected card again keeps it selected', (
    tester,
  ) async {
    await pumpScreen(tester);
  ...
  group('navigation', () {
    testWidgets('carries the completed data to step 4', (tester) async {
      ...
      expect(data.goal, KetoGoal.athleticPerformance);
    });
